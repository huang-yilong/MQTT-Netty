...
![image-20220303005704228](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20220303005704228.png)
...
![image-20220302090700101](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20220302090700101.png)
...
在说GroovyClassLoader之前，我们需要先了解Java的classLoader，也即类的加载器，它使JVM可以动态的载入Java类，JVM并不需要知道从什么地方（本地文件、网络等）载入Java类，这些都由ClassLoader完成。
...
而当我们调用Groovy脚本生成class时，它的类加载器从下至上依次是：GroovyClassLoader.InnerLoader、GroovyClassLoader以及RootLoader。

其中，RootLoader作为Groovy的根ClassLoader，负责加载Groovy及其依赖的第三方库中的类，而且值得注意的是RootLoader没有遵循双亲委派模型，它会先尝试自己去加载这个类，加载不到的话再委派给parent。因为RootLoader的Classpath对Java原生的类加载器是透明的。Java的Classpath只加载Groovy的包，不会加载Groovy依赖的第三方包。

GroovyClassLoader则负责在运行时编译groovy源代码为Class（从而使Groovy实现了将groovy源代码动态加载为Class的功能。）

GroovyClassLoader.InnerLoader在每次编译groovy源代码的时候，都会被新建，它会加载Groovy脚本编译出来的字节码，但是整个加载的工作其实是委派给GroobyClassLoader的，这样做的目的是：
